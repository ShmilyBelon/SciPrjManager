...
void CloseSys(SCH_NODE *hd)
{
    ...
    SCIPRJ_NODE *pSciprjNode1, *pSciprjNode2;

    while (pSchNode1 != NULL) /*释放十字交叉链表的动态存储区*/
    {
        pSchNode2 = pSchNode1->next;
        pTeamNode1 = pSchNode1->tnext;
        while (pTeamNode1 != NULL) /*释放科研团队基本信息支链的动态存储区*/
        {
            pTeamNode2 = pTeamNode1->next;
            pSciprjNode1 = pTeamNode1->snext;
            while (pSciprjNode1 != NULL) /*释放科研项目信息支链的动态存储区*/
            {
                pSciprjNode2 = pSciprjNode1->next;
                free(pSciprjNode1);
                pSciprjNode1 = pSciprjNode2;
            }
            free(pTeamNode1);
